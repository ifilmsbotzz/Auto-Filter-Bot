...
async def start(client, message):
    ...
    if (len(message.command) != 2) or (len(message.command) == 2 and message.command[1] == 'start'):
        buttons = [[
            ...
            InlineKeyboardButton('⚡️ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇꜱ ᴄʜᴀɴɴᴇʟ⚡️', url=UPDATES_LINK)
        ]]
    ...
    if mc.startswith('verify'):
        ...
        await update_verify_status(message.from_user.id, is_verified=True, expire_time=expire_time)
    ...
    if not await db.has_premium_access(message.from_user.id):
        if IS_VERIFY and not verify_status['is_verified']:
            ...
            btn = [[
                InlineKeyboardButton(f'🧿 Verify 🧿', url=f'https://t.me/{temp.U_NAME}?start=verify_{token}')
            ],[
